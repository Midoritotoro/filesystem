...
struct settings {
    ...
    static filesystem_always_inline constexpr auto contains(const _Key_& __keyword) noexcept {
        using found = decltype((std::declval<base>())(_Key_{}));
        return !concepts::same_as<found, unknown_key>;
    }
        
    template <concepts::keyword ... _Keys_>
    static filesystem_always_inline constexpr auto contains_any(_Keys_ ... __keys) noexcept {
        return (contains(__keys) || ...); 
    }

    template <concepts::keyword ... _Keys_>
    static filesystem_always_inline constexpr auto contains_only(const _Keys_& ... __keys) noexcept {
    ...
    template <concepts::keyword... _Keys_>
    static filesystem_always_inline constexpr auto contains_none(_Keys_ ... __keys) noexcept {
        return !contains_any(__keys...); 
    }

    template <concepts::keyword _Key_> 
    constexpr filesystem_always_inline auto operator[](const _Key_& __key) const noexcept {
        return _content(__key);
    }

    template <concepts::keyword ... _Keys_> 
    constexpr filesystem_always_inline auto operator[](const _Keys_& ... __keys) const noexcept {
    ...
    constexpr filesystem_always_inline auto operator[](__type_or<_Key_, _Value_> __value) const {
        if constexpr(contains(_Key_{})) return (*this)[_Key_{}];
        else if constexpr(requires(_Value_ __t) { __t.perform(); }) return __value.value.perform();
        else return __value.value;
    }
