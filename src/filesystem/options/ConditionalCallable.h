...
struct __conditional_callable: callable<_Function_, _OptionsValues_, _Options_...> {
    ...
    constexpr filesystem_always_inline auto behavior(const __Options_& __options,
...
struct relative_conditional_option {
    ...
    {
        return filesystem::options::merge_prefer_first(__base, options{condition_key = __option});
    }

    filesystem_always_inline constexpr auto default_to(const auto& __base) const {
        return filesystem::options::merge_prefer_first(options{condition_key = ignore_none}, __base);
    }
};
        ...
        relative_conditional_option, _Options_...>
{
    ...
    template <
        ...
        class               _Type_,
        class ...           _Types_>
    constexpr filesystem_always_inline auto behavior(
...
struct conditional_option {
    filesystem_always_inline constexpr auto process(
        const auto&                             __base, 
    ...
    filesystem_always_inline constexpr auto process(
        const auto&                     __base, 
        filesystem::vx::simd_mask_type auto   __option) const noexcept
    {
...
{
    using base_t = __conditional_callable<_Function_,  _OptionsValues_, 
        conditional_option, relative_conditional_option, _Options_...>;
