...
constexpr filesystem_always_inline decltype(auto) fetch(const _Keyword_& __keyword, const _Options_& ... __options) noexcept {
...
constexpr filesystem_always_inline decltype(auto) fetch(
    const __type_or<_Keyword_, _Value_>& __keyword, const _Options_& ... __options) noexcept
{
    const auto __opts = settings(__options...);
    return __opts[__keyword];
}
...
constexpr filesystem_always_inline decltype(auto) fetch(const _Key_& __key, const _Settings_& __settings) noexcept {
